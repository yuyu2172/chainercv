...
def record_bbox(model, dataset, device, n_class):
    ...
    for i in range(len(dataset)):
        ...
        if i % 100 == 0:
            ...
            chainercv.visualizations.vis_bbox(img, bbox, ax=ax1)
            ax2 = fig.add_subplot(1, 2, 2)
            print img.shape
            print pred_bbox.astype(np.int32)
            chainercv.visualizations.vis_bbox(
                img, pred_bbox, pred_label, pascal_voc_labels, ax=ax2)
            # plt.show()
